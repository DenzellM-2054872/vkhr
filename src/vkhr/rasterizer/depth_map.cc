...
namespace vkhr {
    namespace vulkan {
        ...
        DepthMap::DepthMap(const std::uint32_t width, const std::uint32_t height,
            ...
            vk::DebugMarker::object_name(vulkan_renderer.m_device, image, VK_OBJECT_TYPE_IMAGE, "Depth Map Image", id);
            ...
            vk::DebugMarker::object_name(vulkan_renderer.m_device, memory, VK_OBJECT_TYPE_DEVICE_MEMORY, "Depth Map Device Memory", id);
            ...
            vk::DebugMarker::object_name(vulkan_renderer.m_device, image_view, VK_OBJECT_TYPE_IMAGE_VIEW, "Depth Map Image View", id);

            framebuffer = vk::Framebuffer{
                vulkan_renderer.m_device,
                vulkan_renderer.m_depth_pass,
                image_view, VkExtent2D {
                    width, height
                }
            };

            vk::DebugMarker::object_name(vulkan_renderer.m_device, framebuffer, VK_OBJECT_TYPE_FRAMEBUFFER, "Depth Map Framebuffer", id);
            ...
            vk::DebugMarker::object_name(vulkan_renderer.m_device, sampler, VK_OBJECT_TYPE_SAMPLER, "Depth Map Sampler", id);
        ...
        DepthMap::DepthMap(const std::uint32_t width, vkrhr::V_Raytracer& vulkan_renderer,
            const LightSource& light_source)
            : DepthMap{ width, width,
                          vulkan_renderer,
                          &light_source } { }
        ...
        DepthMap::DepthMap(vkrhr::V_Raytracer& vulkan_renderer)
            : DepthMap{ vulkan_renderer.m_swap_chain.get_width(),
                          vulkan_renderer.m_swap_chain.get_height(),
                          vulkan_renderer, nullptr } { }
