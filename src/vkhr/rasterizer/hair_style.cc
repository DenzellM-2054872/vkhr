...
namespace vkhr {
    namespace vulkan {
        ...
        void HairStyle::load(const vkhr::HairStyle& hair_style,
            ...
            parameters.vertex_count = vertices.count();
            ...
            //transmitance_sampler = vk::Sampler{
            // vulkan_renderer.device,
            // VK_FILTER_LINEAR,      VK_FILTER_LINEAR,
            // VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER,
            // VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER,
            ...
            //vk::DebugMarker::object_name(vulkan_renderer.device, transmitance_sampler, VK_OBJECT_TYPE_SAMPLER, "Hair transmitance Sampler", id);

            //VkDeviceSize transmitance_length = 
            //    parameters.volume_resolution.x *
            //    parameters.volume_resolution.y *
            //    parameters.volume_resolution.z *
            //    sizeof(unsigned char); // bytes.

            //transmitance_volume = vk::DeviceImage{
            //    vulkan_renderer.device,
            //    static_cast<std::uint32_t>(parameters.volume_resolution.x),
            //    static_cast<std::uint32_t>(parameters.volume_resolution.y),
            //    static_cast<std::uint32_t>(parameters.volume_resolution.z),
            ...
            //vk::DebugMarker::object_name(vulkan_renderer.device, transmitance_volume, VK_OBJECT_TYPE_IMAGE, "Hair transmitance Volume", id);

            //transmitance_view = vk::ImageView{
            //    vulkan_renderer.device,
            //    transmitance_volume
            //};

            //vk::DebugMarker::object_name(vulkan_renderer.device, transmitance_view, VK_OBJECT_TYPE_IMAGE_VIEW, "Hair transmitance View", id);

            //variance_sampler = vk::Sampler{
            //    vulkan_renderer.device,
            //    VK_FILTER_LINEAR,      VK_FILTER_LINEAR,
            //    VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER,
            //    VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER,
            ...
            //vk::DebugMarker::object_name(vulkan_renderer.device, variance_sampler, VK_OBJECT_TYPE_SAMPLER, "Hair variance Sampler", id);

            //VkDeviceSize variance_length =
            //    parameters.volume_resolution.x *
            //    parameters.volume_resolution.y *
            //    parameters.volume_resolution.z *
            //    sizeof(unsigned char); // bytes.

            //variance_volume = vk::DeviceImage{
            //    vulkan_renderer.device,
            //    static_cast<std::uint32_t>(parameters.volume_resolution.x),
            //    static_cast<std::uint32_t>(parameters.volume_resolution.y),
            //    static_cast<std::uint32_t>(parameters.volume_resolution.z),
            ...
            //vk::DebugMarker::object_name(vulkan_renderer.device, variance_volume, VK_OBJECT_TYPE_IMAGE, "Hair variance Volume", id);
            ...
            //vk::DebugMarker::object_name(vulkan_renderer.device, variance_view, VK_OBJECT_TYPE_IMAGE_VIEW, "Hair variance View", id);
        ...
        //void HairStyle::gen_DOM(Pipeline& voxel_pipeline, vk::DescriptorSet& descriptor_set, vk::CommandBuffer& command_buffer) {

        //    descriptor_set.write(2, parameter_buffer);
        //    descriptor_set.write(3, vertices);

        //    for (int i = 0; i < (5 * 2); i++) {
        //        descriptor_set.write(5 + i, dom_views[i], dom_samplers[i]);
        //    }

        //    command_buffer.bind_descriptor_set(descriptor_set, voxel_pipeline);
        //    command_buffer.dispatch(dom_width, dom_height);
        //}

        //void HairStyle::dom_pipeline(Pipeline& pipeline, Rasterizer& vulkan_renderer, int dom_layer_count) {
        ...
        //    pipeline.shader_stages.emplace_back(vulkan_renderer.device, SHADER("self-shadowing/deep_opacity_maps.comp"));

        //    vk::DebugMarker::object_name(vulkan_renderer.device, pipeline.shader_stages[0], VK_OBJECT_TYPE_SHADER_MODULE, "deep opacity map shader");

        //    std::vector<vk::DescriptorSet::Binding> descriptor_bindings{
        //        { 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER }, // the info for the camera
        //        { 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER }, // the info for the lights
        //        { 2, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER }, // extra info about the strands
        //        { 3, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER }, // the vertecies of the strands
        //        { 4, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER }  // the info for the params set in the ui
        //    };

        //    int desc_size = descriptor_bindings.size();
        //    for (std::uint32_t i{ 0 }; i < dom_layer_count * 2; ++i)
        //        descriptor_bindings.push_back({ desc_size + i, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE }); //
        ...
        //    vk::DebugMarker::object_name(vulkan_renderer.device, pipeline.compute_pipeline, VK_OBJECT_TYPE_PIPELINE, "Hair Depth Graphics Pipeline");
        //}
        ...
        //    transmitance_volume.transition(command_buffer,
        //        VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
        //        VK_ACCESS_TRANSFER_WRITE_BIT,
        //        VK_IMAGE_LAYOUT_GENERAL,
        //        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        //    descriptor_set.write(2, parameter_buffer);
        //    descriptor_set.write(3, variance_view);
        //    descriptor_set.write(4, transmitance_view);
        //    descriptor_set.write(5, vertices);
        //    descriptor_set.write(6, tangents);
        ...
        void HairStyle::build_Coolchomp_pipeline(Pipeline& pipeline, Rasterizer& vulkan_renderer) {
            ...
            pipeline.fixed_stages.add_vertex_binding({ 0, 0, VK_FORMAT_R32G32B32_SFLOAT, sizeof(glm::vec3) });  //the postion in the vertex shader
            pipeline.fixed_stages.add_vertex_binding({ 1, 1, VK_FORMAT_R32G32B32_SFLOAT, sizeof(glm::vec3) });  //the tangent in the vertex shader
            pipeline.fixed_stages.add_vertex_binding({ 2, 2, VK_FORMAT_R32_SFLOAT,       sizeof(float) });      //the thickness in the vertex shader
            ...
            for (std::uint32_t i{ 0 }; i < light_count; ++i)
                descriptor_bindings.push_back({ desc_size + i, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER });
